...
ROI. In the example below (Figure @ref(fig:fig02)), we selected the
entire image as the ROI using the rectangle tool.
...
model (Figure @ref(fig:fig03)). This allows pamfit to estimate suitable
...
many high-light levels that result in excessive photoinhibition. Also,
currently, `pamfit` is designed to process incremental light curves,
which means that in the protocols that have a recovery measurement at
the end of the light curve the user will need to remove that last point
at this stage.
...
2.  **Save Data** – allows exporting results outside of R in two
    formats: `.csv` and `.tif` (see Figure @ref(fig:fig04)).
...
@ref(fig:fig05)), and the corresponding output showing an
...
When the user selects an ROI (Figure @ref(fig:fig07)), the `ROI` tab
provides the option to plot two images against each other and displays a
boxplot analysis for each variable (Figure @ref(fig:fig08)).
...
plot the `alpha` image (Figure @ref(fig:alpha)).
...
values distribution (Figure @ref(fig:alphahist)):
...
# Calculating other fluorescence based parameters from `pamfit` results

## Importing results from a `pamfit` TIFF file
