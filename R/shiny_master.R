...
server <- function(input, output, session){
  ...
  output$exportData <- shiny::renderUI({
    shiny::actionButton('exportGE', 'Export to R', width = '80%')
  })
  ...
  output$saveData <- shiny::renderUI({
    shiny::actionButton('save_all', 'Save Data', width = '80%')
  })
  ...
  output$downloadData <- shiny::downloadHandler(
    ...
    content = function(file) {
      ...
      if(input$parameter.space == 'All data' && input$output.types == 'TIFF file(s)'){
        ...
        raster::writeRaster(all.stack, filename = file, format = 'GTiff')

      }
      #TIFF data - not all data
      if(input$parameter.space != 'All data' && input$output.types == 'TIFF file(s)'){
        mod.param <- input$parameter.space
        all.stack <- raster::raster(app.data$model.outputs, layer = mod.param) #only works for model params (not raw data) at the mo
        raster::writeRaster(all.stack, filename = file, format = 'GTiff')
      }
